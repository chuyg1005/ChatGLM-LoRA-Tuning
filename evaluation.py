"""evaluate large language model on the task of relation *extraction* with metric of micro f1 score."""
import os
import torch
import json
...
def compute_f1_score(preds, golds):
    ...
    for pred, gold in zip(preds, golds):
        ...
        pred_triples = set([tuple(triple.split("\t")) for triple in pred_triples if len(triple.split("\t")) == 3])
        gold_triples = set([tuple(triple.split("\t")) for triple in gold_triples])
...
def main(cmd_args):
    if cmd_args.pretrain_dir is not None:
        dirname = f"{cmd_args.pretrain_dir}##{cmd_args.task_dir}"
    else:
        dirname = cmd_args.task_dir
    save_dir = "./checkpoint/{}/train_deepspeed/{}/{}".format(cmd_args.data_name,
                                                              dirname,
                                                              cmd_args.model_name)

    test_path = "./data/{}/instruct_data/{}/test.txt".format(cmd_args.data_name, cmd_args.task_dir)
    ...
    if not cmd_args.zero_shot:
        model = PeftModel.from_pretrained(model, save_dir, torch_dtype=torch.float32,
    ...
    if not cmd_args.zero_shot:
        cache_file = "./cache/{}_{}_{}.jsonl".format(cmd_args.data_name, dirname, cmd_args.model_name)
    else:
        cache_file = "./cache/{}_{}_{}_zero_shot.jsonl".format(cmd_args.data_name, cmd_args.task_dir,
                                                               cmd_args.model_name)
    preds, golds = [], []
    ...
    f1 = compute_f1_score(preds, golds)
    print("f1 score: ", f1)
